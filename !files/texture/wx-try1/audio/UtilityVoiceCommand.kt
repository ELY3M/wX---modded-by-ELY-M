...
object UtilityVoiceCommand {
    ...
    fun processCommand(context: Context, view: View, vrStringF: String, rid1F: String, nws1CurrentF: String, nws1StateCurrent: String): Boolean {
        ...
        if (vrString.contains("radar")) {
            ...
            if (validRid) ObjectIntent(context, WXGLRadarActivity::class.java, WXGLRadarActivity.RID, arrayOf(rid1, nws1StateCurrent))
            ...
            if (nws1Current == "WPC") {
                ...
                ObjectIntent(context, AFDActivity::class.java, AFDActivity.URL, arrayOf(nws1Current, "AFD", "sound"))
            }
        } else if (vrString.contains("cloud")) {
            if (tokens.size > 1) {
                nws1Current = tokens[1]
            }
            ObjectIntent(context, USNWSGOESActivity::class.java, USNWSGOESActivity.RID, arrayOf("nws", nws1Current.toLowerCase(Locale.US)))
        } else if (vrString.toUpperCase(Locale.US).contains("SPC")) {
            if (tokens.size > 1) {
                when {
                    vrString.contains("1") -> ObjectIntent(context, SPCSWOActivity::class.java, SPCSWOActivity.NO, arrayOf("1", "sound"))
                    vrString.contains("2") -> ObjectIntent(context, SPCSWOActivity::class.java, SPCSWOActivity.NO, arrayOf("2", "sound"))
                    vrString.contains("3") -> ObjectIntent(context, SPCSWOActivity::class.java, SPCSWOActivity.NO, arrayOf("3", "sound"))
                    else -> ObjectIntent(context, SPCSWOActivity::class.java, SPCSWOActivity.NO, arrayOf("4-8", "sound"))
                }
            } else {
                ObjectIntent(context, SPCSWOActivity::class.java, SPCSWOActivity.NO, arrayOf("1", "sound"))
            }
        } else if (vrString.contains("day one")) {
            ObjectIntent(context, SPCSWOActivity::class.java, SPCSWOActivity.NO, arrayOf("1", "sound"))
        } else if (vrString.contains("day 2")) {
            ObjectIntent(context, SPCSWOActivity::class.java, SPCSWOActivity.NO, arrayOf("2", "sound"))
        } else if (vrString.contains("day 3")) {
            ObjectIntent(context, SPCSWOActivity::class.java, SPCSWOActivity.NO, arrayOf("3", "sound"))
        } else if (vrString.contains("day 4")) {
            ObjectIntent(context, SPCSWOActivity::class.java, SPCSWOActivity.NO, arrayOf("4-8", "sound"))
        } else if (vrString.contains("add")) {
            vrString = vrString.replace("add location", "")
            ObjectIntent(context, SettingsLocationGenericActivity::class.java, SettingsLocationGenericActivity.LOC_NUM, arrayOf((Location.numLocations + 1).toString(), vrString))
        } else if (vrString.contains("edit")) {
            vrString = vrString.replace("edit location", "")
            ObjectIntent(context, SettingsLocationGenericActivity::class.java, SettingsLocationGenericActivity.LOC_NUM, arrayOf(Location.currentLocationStr, vrString))
            ...
            ObjectIntent(context, WebscreenAB::class.java, WebscreenAB.URL, arrayOf("<a class=\"twitter-timeline\" data-dnt=\"true\" href=\"https://twitter.com/search?q=%23" + stateCodeCurrent.toLowerCase(Locale.US) + "wx\" data-widget-id=\"" + twitterStateId + "\" data-chrome=\"noscrollbar noheader nofooter noborders  \" data-tweet-limit=20>Tweets about \"#" + stateCodeCurrent.toLowerCase(Locale.US) + "wx\"</a><script>!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0],p=/^http:/.test(d.location)?'http':'https';if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src=p+\"://platform.twitter.com/widgets.js\";fjs.parentNode.insertBefore(js,fjs);}}(document,\"script\",\"twitter-wjs\");</script>", "twitter: " + stateCodeCurrent.toLowerCase(Locale.US)))
