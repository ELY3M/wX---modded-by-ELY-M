...
JNIEXPORT jint JNICALL Java_joshuatee_wx_JNI_spotter
(JNIEnv * env, jclass clazz, jobject loc_buff, jobject color_buff,jobject index_buff,
		...
		jfloat y_image_center_pixels, jfloat scale_factor,  jfloat one_degree_scale_factor, jdoubleArray x,jdoubleArray y,jint count,jfloat len,jbooleanArray col, int triangleAmount)
{
	...
	for (i_count = 0; i_count < count; i_count++)
	{
		...
		for(  i = 0; i < triangleAmount;i++) {
			...
			lBuff[l_count + 3 ] = -pix_y_d  + (len * sin(i * twicePi / triangleAmount));

			lBuff[l_count + 4 ] = pix_x_d + (len * cos((i+1) *  twicePi / triangleAmount));
			lBuff[l_count + 5 ] = -pix_y_d  + (len * sin((i+1) * twicePi / triangleAmount));
			...
			c_count += 12; // was 3points*4 is now 4*7
