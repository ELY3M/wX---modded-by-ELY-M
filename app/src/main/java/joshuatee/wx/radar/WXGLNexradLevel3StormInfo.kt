...
internal object WXGLNexradLevel3StormInfo {
    ...
    fun decodeAndPlot(context: Context, fnSuffix: String, rid: String, provider: ProjectionType): List<Double> {
        ...
        if (posnNumbers.size == motNumbers.size && posnNumbers.size > 1) {
            ...
            while (s < posnNumbers.size) {
                ...
                ecArr.indices.forEach { z ->
                    ecArr[z] = ecc.calculateEndingGlobalCoordinates(ExternalEllipsoid.WGS84, start, degree2 + degreeShift, nm2 * 1852.0 * z.toDouble() * 0.25, bearing) // was z+1, now z
