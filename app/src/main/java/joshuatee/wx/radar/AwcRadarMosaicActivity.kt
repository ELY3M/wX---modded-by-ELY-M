...
class AwcRadarMosaicActivity : VideoRecordActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        navDrawer.index = UtilityAwcRadarMosaic.sectors.indexOf(sector)
        getContent(product)
    }

    private fun setupUI() {
        navDrawer = NavDrawer(this, UtilityAwcRadarMosaic.labels, UtilityAwcRadarMosaic.sectors) { getContent(product) }
        image = TouchImage(this, toolbar, R.id.iv, navDrawer, "")
        objectAnimate = ObjectAnimate(this, image)
        objectToolbar.connectClick { navDrawer.open() }
        image.setMaxZoom(8.0f)
        image.connect(navDrawer) { getContent(product) }
    }
    ...
    private fun getContent(productLocal: String = product) {
        ...
        toolbar.subtitle = navDrawer.getLabel()
        title = product
        FutureBytes(this, UtilityAwcRadarMosaic.get(navDrawer.url, product), ::showImage)
    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        when (item.itemId) {
            R.id.action_animate -> objectAnimate.animateClicked(::getContent) { UtilityAwcRadarMosaic.getAnimation(navDrawer.url, product) }
