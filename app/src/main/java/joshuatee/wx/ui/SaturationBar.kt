...
class SaturationBar : View {
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        if (!isInEditMode) {
            shader = LinearGradient(mBarPointerHaloRadius.toFloat(), 0f,
                    x1.toFloat(), y1.toFloat(), intArrayOf(Color.WHITE, Color.HSVToColor(0xFF, mHSVColor)), null,
                    Shader.TileMode.CLAMP)
        } else {
            shader = LinearGradient(mBarPointerHaloRadius.toFloat(), 0f,
                    x1.toFloat(), y1.toFloat(), intArrayOf(Color.WHITE, 0xff81ff00.toInt()), null, Shader.TileMode.CLAMP)
    ...
    var color: Int
        ...
        set(color) {
            ...
            shader = LinearGradient(mBarPointerHaloRadius.toFloat(), 0f, x1.toFloat(), y1.toFloat(), intArrayOf(Color.WHITE, color), null, Shader.TileMode.CLAMP)
            mBarPaint!!.shader = shader
            calculateColor(mBarPointerPosition)
            mBarPointerPaint!!.color = mColor
            if (mPicker != null) {
