...
class OpacityBar : View {
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        if (!isInEditMode) {
            shader = LinearGradient(mBarPointerHaloRadius.toFloat(), 0f,
                    x1.toFloat(), y1.toFloat(), intArrayOf(Color.HSVToColor(0x00, mHSVColor), Color.HSVToColor(0xFF, mHSVColor)), null,
                    Shader.TileMode.CLAMP)
        } else {
            shader = LinearGradient(mBarPointerHaloRadius.toFloat(), 0f,
                    x1.toFloat(), y1.toFloat(), intArrayOf(0x0081ff00, 0xff81ff00.toInt()), null, Shader.TileMode.CLAMP)
    ...
    var opacity: Int
        get() {
            val opacity = Math.round(mPosToOpacFactor * (mBarPointerPosition - mBarPointerHaloRadius))
            return when {
                opacity < 5 -> 0x00
                opacity > 250 -> 0xFF
                else -> opacity
    ...
    var color: Int
        ...
        set(color) {
            ...
            shader = LinearGradient(mBarPointerHaloRadius.toFloat(), 0f,
                    x1.toFloat(), y1.toFloat(), intArrayOf(Color.HSVToColor(0x00, mHSVColor), color), null,
                    Shader.TileMode.CLAMP)
            mBarPaint!!.shader = shader
            calculateColor(mBarPointerPosition)
            mBarPointerPaint!!.color = mColor
