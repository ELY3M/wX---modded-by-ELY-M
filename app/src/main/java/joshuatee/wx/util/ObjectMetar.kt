...
internal class ObjectMetar {
    ...
    constructor(context: Context, location: LatLon) {
        //val obsClosest = UtilityUSv2.getObsFromLatLon(location)
        val obsClosest = UtilityMetar.findClosestObservation(context, location)
        UtilityUSv2.obsClosestClass = obsClosest.name

        //val observationData = ("https://api.weather.gov/stations/" + obsClosest +  "/observations/current").getNwsHtml()
        ...
        val metarData = ("http://tgftp.nws.noaa.gov/data/observations/metar/decoded/" + obsClosest.name + ".TXT").getHtmlSep().replace("<br>", MyApplication.newline)
